...
exports.run = async (client, message, args) => {
  ...
  const bakiye = await db.fetch(`cüzdan_${silinecekkllnc.id}`);
  const hesapdurumu = await db.fetch(`cüzdan_${silinecekkllnc.id}`);
  const hesapismi = await db.fetch(`cüzdan_${silinecekkllnc.id}`);
  
  if(!hesapdurumu) return message.channel.send(`Kayıtlı olan bir kullanıcı belirtmelisin!`)
  await db.add(`cüzdan_${silinecekkllnc.id}`, para)
